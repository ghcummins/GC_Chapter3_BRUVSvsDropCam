...
# Script: Cleaning Pt Cloates BRUVS campaigns
# Task:    Merging BRUVS campaigns and exporting .csv for PRIMER
# author:  G Cummins 
# date:    Oct 2023
##
...
metadata <-ga.list.files("_Metadata.csv")%>% # list all files ending in "_Metadata.csv"
  ...
  dplyr::select(campaignid,sample,latitude,longitude,date,time,location,status,site,depth,observer.count,successful.count,successful.length)%>%
  dplyr::filter(campaignid%in%c("2021-05_PtCloates_BRUVS", "2022-05_PtCloates_stereo-BRUVS"))%>%# This line ONLY keep the 15 columns listed. Remove or turn this line off to keep all columns (Turn off with a # at the front).
  glimpse()
...
points<-as.data.frame(points.files)%>%
  ...
  dplyr::filter(campaignid%in%c("2021-05_PtCloates_BRUVS",
