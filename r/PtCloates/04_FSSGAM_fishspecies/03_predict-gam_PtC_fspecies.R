...
Parapercis_nebulosa_BOSSBRUV <- gam(number ~ s(z, k=3, bs = "cr", by = method) + s(reef, k=3, bs = "cr", by = method) + method,
...
testdata2_pn <- expand.grid(reef = seq(min(dat_total_pn$reef), max(dat_total_pn$reef), length.out = 20),
                            method = c("BOSS", "BRUV"),
                            z = mean(Parapercis_nebulosa_BOSSBRUV$model$z)) %>%
  distinct() %>%
  glimpse()

P.n.fits2 <- predict.gam(Parapercis_nebulosa_BOSSBRUV, newdata = testdata2_pn, type = 'response', se.fit = T)

predicts_pn_reef <- testdata2_pn %>%
  data.frame(P.n.fits2) %>%
