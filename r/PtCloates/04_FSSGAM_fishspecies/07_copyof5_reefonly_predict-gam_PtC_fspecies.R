...
Parapercis_nebulosa_BOSSBRUV <- gam(number ~ s(reef, k=3, bs = "cr", by = method)  + method,
...
testdata_pn <- expand.grid(reef = seq(min(dat_total_pn$reef), max(dat_total_pn$reef), length.out = 20),
...
gg_P_nebulosa_reef <- ggplot() + 
  ...
  theme_classic() +
  annotation_custom(p.neb_grob, xmin = -0.35, xmax = -0.25, ymin = -Inf, ymax = Inf) +
  annotation_custom(p.neb_text, xmin = -0.38, xmax = -0.28, ymin = 0.9, ymax = 0.9) + # Text annotation outside the plot
  theme_void() +
...
gg_L_miniatus_reef <- ggplot() + 
  ...
  annotation_custom(l.min_grob, xmin = -0.35, xmax = -0.25, ymin = -Inf, ymax = Inf) +
  annotation_custom(l.min_text, xmin = -0.38, xmax = -0.29, ymin = 1.6, ymax = 1.6) + # Text annotation outside the plot
   theme_void() +
  coord_cartesian(clip = "off") +
...
reefonly_PtCloates <- gg_P_nebulosa_reef +  gg_L_miniatus_reef + (plot_layout(ncol=1))
reefonly_PtCloates

ggsave(filename = "plots/PtCloates/Residual_plots/reefonly/reefonlyPtCloates3.png", 
       plot = reefonly_PtCloates, 
       width = 8, 
       height = 10, 
       dpi = 600, 
       units = "in")
