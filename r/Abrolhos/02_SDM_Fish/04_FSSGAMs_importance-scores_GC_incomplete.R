###
# Project: Gabby 3rd Chapter PhD
# Data:    BOSS & BRUV fish, habitat
# Task:    Plotting fish importance scores for SDM model
# author:  Gabbby & Claude
# date:    Nov 2023
##
...
dat1 <- read.csv("outputs/ffsgam_fish/Abrolhos_all.var.imp.csv")%>% #from local copy
  rename(resp.var=X)%>%
...
dat.taxa.npz6 <- dat1 %>%
  ...
  mutate(label=ifelse(predictor=="mean.relief"&resp.var=="Labridae Coris auricularis","X",label))%>%
  mutate(label=ifelse(predictor=="z"&resp.var=="Labridae Coris auricularis","X",label))%>%
  ...
  mutate(label=ifelse(predictor=="detrended"&resp.var=="Labridae Coris auricularis","X",label))%>%
  mutate(label=ifelse(predictor=="roughness"&resp.var=="Labridae Coris auricularis","X",label))%>%
  mutate(label=ifelse(predictor=="sd.relief"&resp.var=="Labridae Coris auricularis","X",label))%>%
  glimpse()
