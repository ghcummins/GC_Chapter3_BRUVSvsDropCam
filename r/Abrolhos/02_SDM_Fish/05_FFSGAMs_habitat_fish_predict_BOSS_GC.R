...
# Data:    BRUVS, BOSS and Habitat data
# Task:    Habitat-Fish modelling + Prediction
# author:  Kingsley and Gabby
# date:    Nov 2023
##
...
# # Assuming BOSSdat is your data frame
# Zero <- data.frame(count = sum(BOSSdat$number == 0))
# 
# # Display the Zero data frame
# print(Zero)                                             #c.a. 60% zeros BOSS maxn
...
#trial <- readRDS("outputs/sw-network_gam-abundance.rds")     


# For Multivariate Environmental Similarity Surfaces
xy <- BOSSdat %>%
...
#Need to add predicted reef only in BRUV predict.
...
#NPZ6
#total abundance
m_coris.a <- gam(number ~ s(z, k = 3, bs = "cr"), 
               data = BOSSdat, 
               family = tw())
summary(m_coris.a)
...
preddf <- cbind(preddf, 
                "p_coris.a" = predict(m_coris.a, preddf, type = "response"))
...
prastsc.a <- rasterFromXYZ(preddf[, c(1, 2, 3)])
plot(prastsc.a)

crs(prastsc.a) <- "+proj=utm +zone=50 +south +datum=WGS84 +units=m +no_defs"
saveRDS(prastsc.a, file = "outputs/ffsgam_fish/abrolhos-fish-c.a-spatial_UTM50.rds")
