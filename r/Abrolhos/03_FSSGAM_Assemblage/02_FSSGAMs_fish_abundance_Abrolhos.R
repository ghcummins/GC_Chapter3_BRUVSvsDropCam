...
dat <- readRDS("data/staging/Abrolhos/Abrolhos.maxn.rds")%>%
  dplyr::mutate(reef =rock+inverts+macroalgae, response = paste(response, method, sep = "_"))%>%
...
savedir <- "outputs/Abrolhos/assemblage"
use.dat <- as.data.frame(dat) 
str(use.dat)

is.na(dat$status) #?problem with so many trues?
