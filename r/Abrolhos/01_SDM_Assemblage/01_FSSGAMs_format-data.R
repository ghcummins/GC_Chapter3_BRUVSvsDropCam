...
# primernpz6 <- npz6maxn %>% 
#   group_by(scientific) %>%
#   dplyr::mutate(sum.maxn = sum(maxn)) %>%
#   ungroup() %>%
#   dplyr::filter(sum.maxn > 0) %>%
...
#     values_fill = 0
#   ) %>%
#  # dplyr::select(-c(`Apogonidae Unknown sp10`, `Blenniidae Unknown spp`, `Carangidae Decapterus spp`)) %>% ### if getting rid of species put back here
#   #left_join(metadata) %>%
#   glimpse()
#   
# #write data to load in to next script
# saveRDS(primernpz6, "data/staging/Abrolhos/Abrolhosprimer.rds")
...
ta.sr <- npz6maxn %>%
  ...
  dplyr::mutate(total.abundance = rowSums(.[, 3:(ncol(.))], na.rm = TRUE ),
                species.richness = rowSums(.[, 3:(ncol(.))] > 0)) %>% #Add in Totals
...
#all looks fine
#write data to load in to next script
saveRDS(SB.maxn, "data/staging/Abrolhos/ShallowBank.maxn.rds")
write.csv(SB.maxn, "data/staging/Abrolhos/ShallowBank.maxn.csv")
