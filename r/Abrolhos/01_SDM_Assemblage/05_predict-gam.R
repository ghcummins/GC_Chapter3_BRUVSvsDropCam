...
# read in
dat1 <- readRDS("data/staging/Abrolhos/ShallowBank.maxn.rds")%>%
  dplyr::mutate(reef = macroalgae+rock+inverts)%>%
...
pred.vars <- c("z", "reef",
               "mean.relief","tpi","roughness","detrended") 
...
#Total abundance
m_BOSS_ta <-gam(maxn ~ s(mean.relief, k=3, bs = "cr"),
...
#Species richness
m_BOSS_richness <- gam(maxn ~ s(mean.relief, k = 3, bs = "cr"),
...
saveRDS(preddf, paste0("outputs/Abrolhos/", name, "_predicted-fish.RDS"))
