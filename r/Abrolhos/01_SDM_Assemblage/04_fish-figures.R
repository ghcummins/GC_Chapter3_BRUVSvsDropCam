...
dat <- readRDS(paste0("outputs/Abrolhos/", name, "_predicted-fish.RDS")) %>%
...
habi    <- readRDS(paste(paste0('data/staging/Abrolhos/Abrolhos_habitat-bathy-derivatives.rds')) )%>%
  glimpse()
...
p1 <- ggplot() +
  geom_tile(data = dat %>% filter(z >= 26 & z <=189), aes(x, y, fill = p_BRUV_ta)) +
  scale_fill_viridis(direction =-1, limits= c(-4.3,200))+
  ...
  labs(x = NULL, y = NULL, fill = "ΣMaxN",                                    # Labels  
       colour = NULL, title = "BRUV predicted fish abundance at Shallow Bank, Abrolhos") +
...
print(p1)
png(filename = "plots/Abrolhos/Abrolhos_BRUV_ta.png", 
...
p2 <- ggplot() +
  geom_tile(data = dat %>% filter(z >= 26 & z <=189), aes(x, y, fill = p_BOSS_ta)) +
  scale_fill_viridis(direction = -1, na.value = "transparent", limits= c(-4,200))+
  ...
  coord_sf(xlim = c(113.169637818, 113.65),                              # Set plot limits
           ylim = c(-28.15, -27.95)) +
  labs(x = NULL, y = NULL, fill = "ΣMaxN",                                    # Labels  
       colour = NULL, title = "BOSS predicted fish abundance at Shallow Bank, Abrolhos") +
  annotate("text", x = c(113.428836237, 113.388204915, 113.255153069),          # Add contour labels manually
           y = c(-28.078038504, -28.078038504, -28.078038504), 
           label = c("30m", "70m", "200m"),
           size = 2, colour = "#000000") +
...
print(p2)
png(filename = "plots/Abrolhos/Abrolhos_BOSS_ta.png", 
...
p3 <- ggplot() +
  geom_tile(data = dat %>% filter(z >= 26 & z <=189), aes(x, y, fill = p_BRUV_richness)) +
  scale_fill_viridis(direction =-1, na.value = "transparent", limits= c(-1,13)) +
  geom_contour(data = bathdf, aes(x = x, y = y, z = Z),                         # Contour lines
               breaks = c(- 30, -70, - 200),                                 # Contour breaks - change to binwidth for regular contours
               colour = "#000000",
               alpha = 1, size = 0.5) + 
  ...
  labs(x = NULL, y = NULL, fill = "N. fish species",                                    # Labels  
       colour = NULL, title = "BRUV predicted species richness at Shallow Bank, Abrolhos") +
  annotate("text", x = c(113.428836237, 113.388204915, 113.255153069),          # Add contour labels manually
           y = c(-28.078038504, -28.078038504, -28.078038504), 
           label = c("30m", "70m", "200m"),
           size = 2, colour = "#000000") +
...
print(p3)
png(filename = "plots/Abrolhos/Abrolhos_BRUV_richness.png", 
...
p4 <- ggplot() +
  geom_tile(data = dat %>% filter(z >= 26 & z <=189), aes(x, y, fill = p_BOSS_richness)) +
  scale_fill_viridis(direction =-1, na.value = "transparent", limits= c(-1,13)) +
  geom_contour(data = bathdf, aes(x = x, y = y, z = Z),                         # Contour lines
               breaks = c(- 30, -70, - 200),                                 # Contour breaks - change to binwidth for regular contours
               colour = "#000000",
  ...
  coord_sf(xlim = c(113.169637818, 113.65),                              # Set plot limits
           ylim = c(-28.15, -27.95)) +
  labs(x = NULL, y = NULL, fill = "N. fish species",                                    # Labels  
       colour = NULL, title = "BOSS predicted species richness at Shallow Bank, Abrolhos") +
  annotate("text", x = c(113.428836237, 113.388204915, 113.255153069),          # Add contour labels manually
           y = c(-28.078038504, -28.078038504, -28.078038504), 
           label = c("30m", "70m", "200m"),
           size = 2, colour = "#000000") +
