...
#note gymnocranius grandoculis already labelled that (not sp1 so no need to edit here)
...
Southwest_aspectratio_bruvs <- joiningbruvs %>%
  dplyr::select(-fishbase_scientific)%>%
  rename(name = caab_scientific)%>%
  dplyr::select(name, everything()) 

 write.csv(Southwest_aspectratio_bruvs, file = "outputs/SwC/SouthwestaspectratioBRUVS.csv", row.names = FALSE)
 
 ##summary statistics for Southwest aspect ratio BRUV (this is our BRUV species w aspect ratio)
 #Note this is all species (not all individual fish)
 mean_swc_ar_bruv <- mean(Southwest_aspectratio_bruvs$armean, na.rm = TRUE)
 median_swc_ar_bruv <- median(Southwest_aspectratio_bruvs$armean, na.rm = TRUE)
 ...
 Southwest_aspectratio_boss <- joiningboss %>%
   dplyr::select(-fishbase_scientific)%>%
   rename(name = caab_scientific)%>%
   dplyr::select(name, everything())
 
 write.csv(Southwest_aspectratio_boss, file = "outputs/SwC/SouthwestaspectratioBOSS.csv", row.names = FALSE)
 
 ##summary statistics for Pt Cloates aspect ratio BOSS (this is our BOSS species w aspect ratio)
 #Note this is all species (not all individual fish)
 mean_swc_ar_boss <- mean(Southwest_aspectratio_boss$armean, na.rm = TRUE)
 median_swc_ar_boss <- median(Southwest_aspectratio_boss$armean, na.rm = TRUE)
 ...
 bruv_individuals_aspectratio <- left_join(bruv_individuals_expanded, Southwest_aspectratio_bruvs, by = "name")
 
 write.csv(bruv_individuals_aspectratio, file = "outputs/SwC/SouthwestindividualsaspratioBRUV.csv", row.names = FALSE)
 
 #SUMMARY STATISTICS for BRUV all individuals and aspect ratio
 mean_swc_indi_ar_bruv <- mean(bruv_individuals_aspectratio$armean, na.rm = TRUE)
 median_swc_indi_ar_bruv <- median(bruv_individuals_aspectratio$armean, na.rm = TRUE)
 perc25_swc_indi_ar_bruv <- quantile(bruv_individuals_aspectratio$armean, probs = 0.25, na.rm = TRUE)
 perc75_swc_indi_ar_bruv <- quantile(bruv_individuals_aspectratio$armean, probs = 0.75, na.rm = TRUE)
 ...
 #leftjoin w aspect ratio
 boss_individuals_aspectratio <- left_join(boss_individuals_expanded, Southwest_aspectratio_boss, by = "name")
 ...
 SouthwestarBRUV <- dplyr::select(Southwest_aspectratio_bruvs, name, armean)
 
 #scaled ubiquity and aspect ratio dataframe for BRUVS  
 su_ar_df_BRUV<- left_join(suBRUV, SouthwestarBRUV, by = "name")%>%
   rename(n_BRUV = n)%>%
 ...
 SouthwestsarBOSS <- dplyr::select(Southwest_aspectratio_boss, name, armean)
 
 #scaled ubiquity and aspect ratio dataframe for BOSS
 su_ar_df_BOSS<- left_join(suBOSS, SouthwestsarBOSS, by = "name") %>%
   rename(n_BOSS = n)%>%
 ...
 write.csv(ubiquity_aspectratio, file = "outputs/SwC/Southwestubiquity_aspectratio.csv", row.names = FALSE)
 
 BOSS_AR_INDIFISH <-1.78   # Replace with actual value
 BRUV_AR_INDIFISH <- 1.74
 ...
 ggsave("Southwest_ubiquplot_complete.png", plot = ubiquplot, path = "plots/" , width = 10, height = 10, dpi = 600, units = "in")  
