...
Pseudolabrus_biserialis_BOSSBRUV <- gam(number ~ s(z, k=3, bs = "cr", by = method) + method,
...
gg_P_biserialis_z <- ggplot() + 
  ...
  # geom_ribbon(data = predicts_total_z, aes(x = z, ymin = number - se.fit, ymax = number + se.fit, fill = method, group = method)) +
   theme_classic() +
  annotation_custom(p.bis_grob, xmin = -7, xmax = 0, ymin = -Inf, ymax = Inf) +
  annotation_custom(p.bis_text, xmin = -16, xmax = -6, ymin = 2.7, ymax = 2.7) + # Text annotation outside the plot
  theme_void() +
...
Opthalmolepsis_lineolatus_BOSSBRUV <- gam(number ~ s(z, k=3, bs = "cr", by = method)  + method,
...
Neatypus_obliquus_BOSSBRUV <- gam(number ~ s(z, k=3, bs = "cr", by = method) + method,
...
gg_N_obliquus_z <- ggplot() + 
  ...
  # geom_ribbon(data = predicts_total_z, aes(x = z, ymin = number - se.fit, ymax = number + se.fit, fill = method, group = method)) +
  theme_classic() +
  annotation_custom(n.obl_grob, xmin = -12, xmax = -3, ymin = -Inf, ymax = Inf) +
  annotation_custom(n.obl_text, xmin = -20, xmax = -10, ymin = 4, ymax = 4) +
  theme_void() +
  coord_cartesian(clip = "off") +
  theme(plot.margin = margin(0.5, 0.5, 0.5, 3.5, "cm" ))+
  #ylim(0,50)+
...
#join altogether
p_swc_depthonly <- gg_P_biserialis_z +  gg_O_lineolatus_z +  gg_C_auricularis_z  +  gg_N_obliquus_z +  plot_layout(ncol = 1)
p_swc_depthonly

#save
ggsave(filename = "plots/SwC/BOSSBRUV_Southwest_depthonly_effectsplots.png", 
       plot = p_swc_depthonly, 
       width = 12, 
       height = 16, 
       dpi = 600, 
       units = "in")
...
#save
ggsave(filename = "plots/SwC/depthonly/BOSSBRUV_Southwest_effectsplots_depthonly1.png", 
       plot = depth_swc, 
       width = 7, 
       height = 16, 
       dpi = 600, 
